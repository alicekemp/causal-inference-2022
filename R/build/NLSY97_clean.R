...
read_csv(here("data/NLSY97_raw.csv")) %>%
  ...
  rowwise() %>%
  mutate(incarceration_length = sum(c_across(starts_with("E")), na.rm = TRUE)) %>%
  # include only indivs who were incarcerated for at least one month
  filter(incarceration_length > 0) %>%
  ungroup() %>%
  ...
  select(race, gender, incarceration_length) %>%
